...
For Kubernetes, Docker Compose, or other setups, visit the [Open WebUI Documentation](https://docs.openwebui.com/getting-started/). Note that you will need to build the image from source to include the custom chunking algorithm.
...
```bash
docker run -d --network=host -v open-webui-data:/app/backend/data -e OLLAMA_BASE_URL=http://127.0.0.1:11434 --name open-webui --restart always open-webui
```

### Keeping Your Docker Installation Up-to-Date

Pull the latest changes, rebuild the image and restart the container:
